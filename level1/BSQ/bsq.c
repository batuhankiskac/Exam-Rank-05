...
static int load_elements(FILE *file, t_elements *e)
{
	if (fscanf(file, "%d %c %c %c", &e->n_lines, &e->empty, &e->obstacle, &e->full) != 4)
		return -1;
	if (e->n_lines <= 0) return -1;
	if (e->empty == e->obstacle || e->empty == e->full || e->obstacle == e->full)
		return -1;
	if (e->empty < 32 || e->empty > 126) return -1;
	if (e->obstacle < 32 || e->obstacle > 126) return -1;
	if (e->full < 32 || e->full > 126) return -1;
	return 0;
}
...
static int load_map(FILE *file, t_map *map, t_elements *e)
{
	...
	char *line = NULL;
	size_t len = 0;
	if (getline(&line, &len, file) == -1) { free(line); free_map(map->grid); return -1; }

	for (int i = 0; i < map->height; i++)
	{
		int r = getline(&line, &len, file);
		if (r == -1 || line[r - 1] != '\n') { free(line); free_map(map->grid); return -1; }
		r--;
		if (i == 0) map->width = r;
		else if (map->width != r) { free(line); free_map(map->grid); return -1; }

		map->grid[i] = malloc(r + 1);
		if (!map->grid[i]) { free(line); free_map(map->grid); return -1; }
		for (int k = 0; k < r; k++) map->grid[i][k] = line[k];
		map->grid[i][r] = '\0';
	}
	free(line);

	if (valid_chars(map->grid, e->empty, e->obstacle) == -1) { free_map(map->grid); return -1; }
	return 0;
}
...
{
	t_elements e;
	if (load_elements(file, &e) == -1) return -1;

	t_map map;
	if (load_map(file, &map, &e) == -1) return -1;
